...
class App:
    ...
    def instantiate_varibales(self):
        ...
        self.profile_area= list()
        self.profile_link = list()

    def write_captions_to_excel_file(self, i):
        mydict = dict()
        mydict['profile_name'] = self.profile_name
        mydict['profile_headline'] = self.profile_headline
        mydict['profile_area'] = self.profile_area
        mydict['profile_link'] = self.profile_link
        elem = pd.DataFrame(mydict)
        elem.to_excel(f'Data/Connections_data_{i}.xlsx', index=False)
        self.instantiate_varibales()
        gc.collect()
    ...
    def get_profile_links(self):
        ...
        sleep(2)
        for i in tqdm(range(1,iterator+1)):
            ...
            sleep(2)
            profile_names= self.driver.find_elements_by_xpath('//li[@class="search-result search-result__occluded-item ember-view"]/div/div/div[2]/a/h3/span/span/span[1]')
            profile_names= [name.get_attribute('textContent') for name in profile_names]
            profile_headlines= self.driver.find_elements_by_xpath('//li[@class="search-result search-result__occluded-item ember-view"]/div/div/div[2]/p[1]/span')
            profile_headlines= [name.get_attribute('textContent') for name in profile_headlines]
            profile_area= self.driver.find_elements_by_xpath('//li[@class="search-result search-result__occluded-item ember-view"]/div/div/div[2]/p[2]/span')
            profile_area= [name.get_attribute('textContent') for name in profile_area]
            ...
            self.profile_name.extend(profile_names)
            self.profile_headline.extend(profile_headlines)
            self.profile_area.extend(profile_area)
            self.profile_link.extend(profile_links)
            if i % 10 == 0:
                self.write_captions_to_excel_file((i)//10)
